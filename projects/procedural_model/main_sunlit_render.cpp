...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        for (int lightPos = 0; lightPos < 100; lightPos++){
            ...
            context.setPrimitiveData(UUIDs_leaves, "reflectivity_spectrum", "ColorReference_Calibrite_04"); //green
            context.setPrimitiveData( UUIDs_leaves, "specular_exponent", 10.f);
            std::vector<uint> UUIDs_all = plantarchitecture.getAllUUIDs();
            context.setPrimitiveData(UUIDs_all, "reflectivity_spectrum", "ColorReference_Calibrite_04"); //green
            context.setPrimitiveData( UUIDs_all, "specular_exponent", 10.f);
            context.setPrimitiveData( UUIDs_tile, "reflectivity_spectrum", "ColorReference_Calibrite_13");
            RadiationModel radiationc(&context);
            ...
            uint sunID = radiationc.addSunSphereRadiationSource( sun_dir );
            radiationc.setSourceSpectrum( sunID, "solar_spectrum_ASTMG173");
            double light_anglec = 2.0 * 3.14159265358 * lightPos / 100; // evenly spaced angles
            vec3 supplementary_lightc(1.25 * cos(light_anglec), 1.25 * sin(light_anglec), 1.8f);
            uint lightIDc = radiationc.addSphereRadiationSource(supplementary_lightc, 1.f);
            radiationc.setSourceSpectrum( lightIDc, "solar_spectrum_ASTMG173");

            radiationc.addRadiationBand("red");
            radiationc.disableEmission("red");
            radiationc.setDiffuseRadiationExtinctionCoeff( "red", 0.2f, sun_direction);
            radiationc.setScatteringDepth("red", 3);
            radiationc.copyRadiationBand("red", "green");
            radiationc.copyRadiationBand("red", "blue");
            std::vector<std::string> bandlabels = {"red", "green", "blue"};
            radiationc.setDiffuseSpectrum( bandlabels, "solar_spectrum_ASTMG173");
            // CameraCalibration calibration(&context);
            // calibration.addCalibriteColorboard( make_vec3(0,0.23,0.001), 0.025 );
            ...
            radiationc.setCameraSpectralResponse(cameralabel, "red", "iPhone12ProMAX_red");
            radiationc.setCameraSpectralResponse(cameralabel, "green","iPhone12ProMAX_green");
            radiationc.setCameraSpectralResponse(cameralabel, "blue", "iPhone12ProMAX_blue");

            radiationc.updateGeometry();
            radiationc.runBand(bandlabels);

            // Process the image using standard pipeline
            radiationc.applyImageProcessingPipeline("bunnycam", "red", "green", "blue", 1, 1, 1, 1);
            
            // Save the rendered RGB image
            std::string image_file = radiationc.writeCameraImage( cameralabel, bandlabels, "RGB", "../output/", lightPos);
            std::string corrected_image = radiationc.autoCalibrateCameraImage("bunnycam", "red", "green", "blue", image_file, true);
            ...
            radiation.setSourceFlux(lightID, "PAR", 0.5f);
